...
with tab1:
    ...
    vehicle_types_flat = df['vehicle_types'].fillna('').astype(str).str.split(',').explode()
    fig, ax = plt.subplots()
    ...
    features_flat = df['dashboard_features'].fillna('').astype(str).str.split(',').explode()
    fig, ax = plt.subplots()
    ...
    priorities_flat = df['business_priorities'].fillna('').astype(str).str.split(',').explode()
    fig, ax = plt.subplots()
...
with tab2:
    ...
    if pred_file:
        ...
        for col in multi_cols:
            mlb = MultiLabelBinarizer()
            vals = new_enc[col].fillna('').astype(str).str.split(',')
            enc = pd.DataFrame(mlb.fit_transform(vals), columns=[f"{col}_{c}" for c in mlb.classes_])
...
with tab4:
    ...
    for col in [col1, col2]:
        mlb = MultiLabelBinarizer()
        arr = mlb.fit_transform(df_ap[col].fillna('').astype(str).str.split(','))
